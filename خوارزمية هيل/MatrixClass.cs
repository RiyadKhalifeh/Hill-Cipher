...
namespace Mehroz
{
	...
	public class Matrix
	{
		...
		/// </summary>
        /// 
       // public int Determinent() { return 1; }

		public Fraction Determinent()
		{
            if (this.Rows == 1 && this.Cols == 1)
            {
               // Fraction ff= this[0,0];
                return this[0,0];
            }
            else
		...
		public Matrix Adjoint()
		{
            if (this.Rows == 1 && this.Cols == 1)
            {                
                return Matrix.Multiply(this.Inverse(), Convert.ToInt32(this[0,0].Numerator));
            }
            else
            {
                if (this.Rows != this.Cols)
		...
		public static Matrix UnitMatrix(int iRows, int iCols)
{
        ...
        public static Matrix Mod(Matrix a, int n)
        {
            Matrix newArray = a;
            ...
            for (int row = 0; row < a.Rows; row++)
            {
                ...
                {
                    int m = Mod(Convert.ToInt32(a[row, col].Numerator), n);
                    newArray[row, col].Value = m;
                }
            }
            return newArray;
        }   

		public static Matrix Multiply(Matrix matrix, int iNo)
		{
